...
class Char_Test_Data_Handler(Interface_Data_Handler) :
    def __init__(self, app, folder_name, sampling_frequency, settling_time, capture_time, flow_points, log_discrete_data) :
        ...
        self.char_data_file_name = r"..\\" + folder_name + "\Liquid Flow Char Test Data " + str(time.strftime("%d %b %Y %H_%M_%S", time.localtime()))
        self.char_data_fields = ['Time [ms]', 'Timestamp', 'Actual Temperature', 'Actual Flow', 'Raw Value', 'Corrected Value', 'Temperature Value',
        ...
        if self.log_discrete_data != 0:
            self.discrete_data_file_name = r"..\\" + folder_name + "\\Discrete Data" + "\\Liquid Flow Discrete Data " + str(time.strftime("%d %b %Y %H_%M_%S", time.localtime()))
            self.discrete_data_fields = ['Time [ms]', 'Timestamp', 'Actual Temperature', 'Actual Flow', 'Raw Value', 'Corrected Value', 'Temperature Value',
...
#                               "LD20 Corrected Flow Value [ml/hr] [Blue]", True)
        self.graph = Data_Plot3(app, self.actual_flow_points, self.honeywell_raw_value, self.LD20_corrected_value,  self.honeywell_corrected_value,
                               "Characterization Plot", "Actual Flow [ml/hr]", "Honeywell Sensor Raw Value [Red]",
    ...
    def add_entry_sensor_data(self, lost_entries, raw_value, corrected_value, temperature_value) :
        ...
        if self.log_discrete_data != 0:
            data = []
            data = [str(self.clock.get_elapsed_milli_sec()), self.clock.get_time_milli_sec(), self.str_temp_point, self.str_flow_point, 
                                                self.float_to_string(raw_value), self.float_to_string(corrected_value), self.float_to_string(temperature_value), 
                                                self.float_to_string(self.sensirion_flow/20.0), self.float_to_string(self.sensirion_temp/200.0), self.float_to_string(self.sensirion_status), 
                                                self.float_to_string(self.v_h), self.float_to_string(self.v_t), self.float_to_string(self.i_h), self.float_to_string(self.i_t), self.float_to_string(self.v_drop_t)]
        ...
        if self.is_one_flow_point_completed == 0:
            if self.is_settling_time_over == 0:
                ...
                if self.clock.is_alarm_went_off() == 0:
                    ...
                    data = []
                    data = [str(self.clock.get_elapsed_milli_sec()), self.clock.get_time_milli_sec(), self.str_temp_point, self.str_flow_point, 
                                                    self.float_to_string(self.avg_raw_value), self.float_to_string(self.avg_corrected_value), self.float_to_string(self.avg_temperature_value), 
                                                    self.float_to_string(self.avg_sensirion_flow), self.float_to_string(self.avg_sensirion_temp)]

                    self.char_data_logger.write_row(data)

                    self.graph.append_values(self.flow_point, self.avg_raw_value, self.avg_sensirion_flow, self.avg_corrected_value)
                    self.graph.update_plot()
...
class Coeffs_Data_Handler(Interface_Data_Handler):
    def __init__(self, app, folder_name) :
        self.app = app
        self.discrete_data_logger_file_name = folder_name + "\\Liquid Flow Monitor Test Data " + str(time.strftime("%d %b %Y %H_%M_%S", time.localtime()))
...
class Monitor_Test_Data_Handler(Interface_Data_Handler) :
    def __init__(self, app, folder_name, sampling_frequency, time_period_to_display_data) :
        ...
        self.discrete_data_fields = ['Time [ms]', 'Timestamp', 'Raw Value', 'Corrected Value', 'Temperature Value',
                                'LD20 Flow', 'LD20 Temperature', 'LD20 Status',
                                'Heater Voltage', 'Thermistor Voltage', 'Heater Current', 'Thermistor Current', 'Series R Voltage']
        self.discrete_data_logger = CSV_Data_Logger(self.discrete_data_logger_file_name, 100000, self.discrete_data_fields)
...
#                               "LD20 Corrected Flow Value [ml/hr] [Blue]", False)
        self.graph = Data_Plot3(app, self.time_points, self.honeywell_raw_value, self.LD20_corrected_value, self.honeywell_corrected_value,
                               "Plot", "Elapsed time [seconds]", "Honeywell Sensor Raw Value [Red]",
    ...
    def add_entry_sensor_data(self, lost_entries, raw_value, corrected_value, temperature_value) :
        ...
        data = [str(self.clock.get_elapsed_milli_sec()), self.clock.get_time_milli_sec(),
                                             self.float_to_string(raw_value), self.float_to_string(corrected_value), self.float_to_string(temperature_value), 
                                             self.float_to_string(self.sensirion_flow), self.float_to_string(self.sensirion_temp/200.0), self.float_to_string(self.sensirion_status),
                                             self.float_to_string(self.v_h), self.float_to_string(self.v_t), self.float_to_string(self.i_h), self.float_to_string(self.i_t), self.float_to_string(self.v_drop_t)]
