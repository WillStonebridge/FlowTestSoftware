...
class Char_Test_Data_Handler(Interface_Data_Handler) :
    def __init__(self, app, folder_name, sampling_frequency, settling_time, capture_time, flow_points, log_discrete_data) :
        ...
        self.char_data_file_name = folder_name + "\\Liquid Flow Char Test Data " + str(time.strftime("%d %b %Y %H_%M_%S", time.localtime()))
        ...
        if self.log_discrete_data != 0:
            self.discrete_data_file_name = folder_name + "\\Discrete Data" + "\\Liquid Flow Discrete Data " + str(time.strftime("%d %b %Y %H_%M_%S", time.localtime()))
...
class Monitor_Test_Data_Handler(Interface_Data_Handler) :
    def __init__(self, app, folder_name, sampling_frequency, time_period_to_display_data) :
        ...
        self.discrete_data_fields = ['Time [ms]', 'Timestamp', 'Raw Value', 'Corrected Value', 'Temperature Value',
                                'LD20 Flow', 'LD20 Temperature', 'LD20 Status',
                                'Heater Voltage', 'Thermistor Voltage', 'Heater Current', 'Thermistor Current', 'Series R Voltage']
        self.discrete_data_logger = CSV_Data_Logger(self.discrete_data_logger_file_name, 310000, self.discrete_data_fields)
...
#                               "LD20 Corrected Flow Value [ml/hr] [Blue]", False)
        ...
        self.graph.set_axes_limits(0, 100, 1500000, 3000000, -350, 350)
