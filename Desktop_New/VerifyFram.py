...
def GetFramContents():
    ...
    try:
        ...
        showerror(title="USB Error", message="An exception occurred while trying to create serial connection.")
        return
    except OSError as e:
        showerror(message="The serial device is suddenly unavailable while trying to create a serial connection: {err}".format(
                err=str(e)))
        return
    ...
    for i in range(0, num_records):
        ...
        s = ser.read(28)
        if s == b'':
            showerror(title="Communication Error", message="Did not receive response to Read FRAM command.")
            return
...
if __name__ == '__main__':
    ...
    #com_port = tkinter.Listbox(root, listvariable=port,  list=ports, state='normal', justify='center', height=1,)
    com_port = tkinter.Spinbox(root, textvariable=port,  values=ports, state='readonly', justify='center',)
    for p in ports:
        com_port.insert('end', p)
